...
import 'pages/home_page.dart';
import 'package:firebase_core/firebase_core.dart'; // Firebase Core package එක import කිරීම
import 'package:firebase_auth/firebase_auth.dart'; // 🎯 Session Persistence සඳහා Auth import කළා
import 'firebase_options.dart'; // 2. අර හැදුණු options file එක Import කරන්න
...
void main() async {
  ...
  // 3. Riverpod වලට අදාළ ProviderScope එක ඇතුළත ඔබේ ඇප් එක රන් කරන්න
  runApp(const ProviderScope(child: TharuBagApp()));
}
...
class TharuBagApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      // 🎯 පරණ `home: const LoginPage()` වෙනුවට මේ අලුත් StreamBuilder එක දැම්මා
      home: StreamBuilder<User?>(
        stream: FirebaseAuth.instance.authStateChanges(),
        builder: (context, snapshot) {
          // ⏳ Firebase එකෙන් කලින් ලොග් වුණු කෙනෙක් ඉන්නවද කියලා චෙක් කරනකම් Loading එකක් පෙන්වයි
          ...
          // 🔓 කලින් ලොග් වුණු යූසර් කෙනෙක් ෆෝන් එකේ ඉන්නවා නම් කෙලින්ම HomePage එකට යවනවා
          if (snapshot.hasData && snapshot.data != null) {
            return const HomePage(); // 💡 ඔයාගේ ප්‍රධාන පේජ් එක HomePage එක නිසා මෙතනට ඒක දුන්නා
          }

          // 🔒 කිසිම කෙනෙක් ලොග් වෙලා නැත්නම් විතරක් Login පේජ් එක පෙන්වනවා
