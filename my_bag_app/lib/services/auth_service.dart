...
class AuthService {
  ...
  Future<User?> registerWithEmail(String email, String password) async {
    try {
      ...
      if (user != null) {
        // පරිශීලකයා ලියාපදිංචි වුණු ගමන් Firestore එකේ 'users' කියලා තැනක සේව් කරනවා
        await _db.collection('users').doc(user.uid).set({
          'uid': user.uid,
          'email': email,
          'role': 'user', // හැමෝම මුලින් 'user' වෙන්නේ
      ...
      // 🎯 මෙතනින් තමයි Error එක RegisterPage එකට විසි කරන්නේ
      throw e;
    } catch (e) {
      print("Register General Error: ${e.toString()}");
      throw Exception("ලියාපදිංචි වීම අසාර්ථකයි!");
    }
  }
